...
import com.xseed.ocvsample.ocvsample.scanbase.pojo.FrameModel;
...
//        Logger.logOCV("onPictureTaken > preview > "+parameters.getPreviewSize().width+","+parameters.getPreviewSize().height);
    ...
    protected void postSuccess(final Bitmap originalBitmap, final Mat originalMat, final SheetOutput finalOutput) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (mSheetListener != null) {
